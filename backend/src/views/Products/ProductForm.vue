...
<template>
  <div class="flex items-center justify-between mb-3">
    <h1 v-if="!loading" class="text-3xl font-semibold">
      {{
        product.id ? `แก้ไขสินค้า : "${product.title}"` : 'Create new Product'
      }}
    </h1>
  </div>
    ...
    store
      ...
      .then((response) => {
        ...
        if (response.status === 200) {
          product.value = response.data;
          store.commit('showToast', 'รายการสินค้าได้รับการแก้ไขเรียบร้อยแล้ว');
    ...
    store
      ...
      .then((response) => {
        ...
        if (response.status === 201) {
          product.value = response.data;
          store.commit('showToast', 'สร้างรายการสินค้าสำเร็จแล้ว');
          store.dispatch('getProducts');
          if (close) {
            router.push({ name: 'app.products' });
          } else {
